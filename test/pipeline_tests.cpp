...
auto sign_message =
    [](const std::pair<std::string, std::string> &msg_name_pair) {
        return std::string(msg_name_pair.first + "\nFrom " +
                           msg_name_pair.second);
    };

TEST(PipelineTest, BuildSimpleStage) {
    Pipeline p;
    Result<Port<int>> src_res = p.add_stage("src", src);
...
TEST(PipelineTest, DuplicateStage) {
    Pipeline p;
    Result<Port<int>> src_res = p.add_stage("src", src);
    Result<Port<int>> err = p.add_stage("src", src);
...
TEST(PipelineTest, WireWithPorts) {
    ...
    Result<Port<int>> incr_res = p.add_stage("incr", incr, src_p);
    ASSERT_TRUE(incr_res.has_value());
    Port<int> incr_p = incr_res.value();

    Result<Port<int>> triple_res = p.add_stage("triple", triple, incr_p);
...
TEST(PipelineTest, RunSingleStage) {
    ...
    auto src_res = p.add_stage("src", src);
    ASSERT_TRUE(src_res.has_value());
...
TEST(PipelineTest, RunMultipleStages) {
    ...
    auto incr_res = p.add_stage("incr", incr, src_p);
    ASSERT_TRUE(incr_res.has_value());
    auto incr_p = incr_res.value();

    auto triple_res = p.add_stage("triple", triple, incr_p);
    ASSERT_TRUE(triple_res.has_value());
    ...
    */
    ...
    auto upper_msg_to_bytes_port =
        p.add_stage("upper_msg_to_bytes", string_to_bytes, upper_msg_port)
            .value();
    auto write_msg_port =
        p.write_bytes_to_file("write_msg", msg_path, upper_msg_to_bytes_port)
            .value();

    auto name_port = p.add_stage("name", name).value();
    auto upper_name_port =
        p.add_stage("upper_name", to_upper, name_port).value();
    auto upper_name_to_bytes_port =
        p.add_stage("upper_name_to_bytes", string_to_bytes, upper_name_port)
            .value();
    auto write_name_port =
        p.write_bytes_to_file("write_name", name_path, upper_name_to_bytes_port)
            .value();

    auto read_name_bytes_port =
        p.read_bytes_from_file("read_name", name_path, write_name_port).value();
    auto read_name_str_port =
        p.add_stage("name to str", bytes_to_string, read_name_bytes_port)
            .value();

    auto read_msg_bytes_port =
        p.read_bytes_from_file("read_msg", msg_path, write_msg_port).value();
    auto read_msg_str_port =
        p.add_stage("msg to str", bytes_to_string, read_msg_bytes_port).value();

    auto join_port =
        p.join("join", read_msg_str_port, read_name_str_port).value();
    auto signature_port =
        p.add_stage("signature", sign_message, join_port).value();
